...
[cite_start]PC 시리얼 터미널 프로그램(`ComPortMaster`)을 연동하여 9600bps 보레이트로 실시간 데이터를 양방향 제어합니다[cite: 61]. 

![UART Terminal Monitor](images/uart_terminal.png)

- [cite_start]**명령어 수신**: PC에서 `start` 및 `stop` 문자열 명령을 전송하면 UART 수신 인터럽트가 발생하여 Circular Queue에 저장 후 비동기 처리됩니다[cite: 41, 50].
- [cite_start]**실시간 로그 송신**: 시스템은 현재 진행 중인 공정 모드 상태와 남은 시간 데이터(`" <Washing> time - 0:59 "`)를 1초 주기로 PC 터미널 창에 실시간 출력합니다[cite: 40, 61].

---

### 📌 오실로스코프 신호 분석 (UART Physical Layer)
[cite_start]동작 신호의 무결성을 확보하기 위해 오실로스코프로 UART 통신선(TX/RX)의 물리 계층 신호를 직접 계측 및 분석하여 하드웨어 동작을 최종 검증했습니다[cite: 5, 61].
...
- [cite_start]**데이터 프레임 파싱 무결성**: 하강 엣지(Falling Edge) 트리거 포인트를 기준으로 `Start Bit (st)` ➔ `Data Bit (8-bit)` ➔ `Stop Bit (sp)` 구조를 분석하여, 실제 데이터 시그널(예: `STOP` 명령어 프레임의 ASCII 매핑 구조)이 하드웨어 레벨에서 왜곡 없이 송수신됨을 검증 완료했습니다[cite: 61].

---

## 6. 트러블슈팅 및 개선점 (Troubleshooting)

[cite_start]프로젝트 빌드 및 하드웨어 통합 테스트 과정에서 발생한 핵심 문제점들과 이를 해결한 최적화 과정입니다[cite: 3, 62].

### ⚠️ 문제점 1: 시간 체크 변수 관리 및 누락 오류
- [cite_start]**현상**: 여러 소스 파일 간에 실시간 시간 카운트 변수를 공유하는 과정에서 타이머 제어가 꼬이거나 변수 초기화가 누락되어 카운트다운이 멈추는 현상 발생[cite: 64].
- [cite_start]**원인**: 소스 파일 간 연동을 위한 `extern` 선언 누락 및 전역 변수의 무분별한 사용으로 인한 시점 제어 오류[cite: 64].
- **해결 방안**: 
  - [cite_start]전역 변수의 사용을 최소화하고 인터페이스 함수 형태로 캡슐화[cite: 68].
  - [cite_start]각 기능 함수들을 명확하게 헤더 파일에 선언하여 참조 누락을 구조적으로 방지[cite: 68].
  - [cite_start]전체 시스템 내에서 공정 제어용 시간 체크 변수를 하나로 통일하여 동기화 성능 개선[cite: 68].

### ⚠️ 문제점 2: 매크로 중복 선언으로 인한 빌드 에러
- [cite_start]**현상**: 기능이 확장됨에 따라 컴파일 시 매크로 중복 정의(Redefinition) 경고 및 빌드 에러 발생[cite: 65].
- [cite_start]**원인**: 소스 파일 및 헤더 파일마다 동일한 하드웨어 제어용 매크로 제어 코드가 개별적으로 선언되어 꼬임 발생[cite: 65].
- [cite_start]**해결 방안**: 프로그래밍 아키텍처 개선을 위해 프로젝트 공통 매크로 및 상수 정의를 전용 헤더 파일인 `define.h` 파일 하나로 모아서 중앙 집중식으로 관리함으로써 해결[cite: 69].

### ⚠️ 문제점 3: 매크로 자료형 연산 오류로 인한 비정상 값 출력
- [cite_start]**현상**: 레지스터 분주비 및 타이머 카운트 계산용 매크로 연산 시, 의도치 않은 쓰레기 값이나 잘못된 타이밍 주기가 계산되어 출력됨[cite: 66].
- [cite_start]**원인**: 16MHz 클럭 연산 시 상수의 크기가 일반 `int` 범위를 초과하거나, 컴파일러의 암시적 형변환에 의해 상위 비트가 손실되는 현상 발생[cite: 66].
- [cite_start]**해결 방안**: 매크로 수식 계산 시 명시적 자료형 변환(`(uint32_t)` 등 매크로 형변환)을 컴파일러에 지시하여 대용량 상수의 연산 무결성을 확보함으로 해결[cite: 70].
