...
### 🎬 전체 시스템 통합 구동 및 신호 검증 데모
> 공정별(세탁➔헹굼➔탈수) FND 애니메이션/모터 PWM 구동과 PC UART 송수신 로그 등 전체 통합 시연 영상입니다.

[[🎬 스마트 세탁기 시스템 통합 구현 영상]](https://github.com/user-attachments/assets/c99f2984-950a-4755-87bf-ce3245d4fa8f)

---

### 📊 주요 기능 및 하드웨어 실측 분석 요약

* **비차단(Non-blocking) 시퀀스 제어**: 세탁, 헹굼, 탈수 각 단계별로 독립적으로 가동 시간이 차감되며, 공정 특성에 맞춘 타겟 PWM 속도로 모터가 가변 제어됩니다.
* **UART 명령어 수신 및 처리**: PC 터미널 프로그램(`ComPortMaster`)에서 무선 제어 문자열(`start`, `stop`)을 송신하면 UART 수신 인터럽트 벡터가 활성화되어 Circular Queue에 유실 없이 적재 후 비동기 제어가 수행됩니다.
* **실시간 로그 출력**: 시스템 내부 커널이 1초 주기로 현재 공정 상태 및 잔여 시간 로그(`" <Washing> time - 0:59 "`)를 가공하여 PC 화면에 실시간 피드백합니다.
* **물리 계층 파싱 무결성**: 오실로스코프 하강 엣지(Falling Edge) 트리거 포인트를 기준으로 실측된 1비트 폭 타이밍이 $104.16\mu\text{s}$ ($1 \div 9600 \approx 104.16\mu\text{s}$)로 완벽히 일치하여, 하드웨어 레벨에서 데이터 왜곡 없이 송수신됨을 최종 검증했습니다.
